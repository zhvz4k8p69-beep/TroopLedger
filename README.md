...
TroopLedger includes Sparkle 2.10.0 for **Check for Updates…** in the app menu and **Preferences → Updates** for automatic checking. The feed and signed archives are hosted in the shared public [mac-updates](https://github.com/zhvz4k8p69-beep/mac-updates) repo; the macOS update-signing key stays in Keychain and iOS distribution is unchanged. `Scripts/release-mac.sh` builds a notarized universal app and `Scripts/prepare-sparkle-update.sh --publish` signs and publishes it — see [the release guide](Scripts/SPARKLE-RELEASES.md).
