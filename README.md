...
## Product fundraiser management

Open **Fundraisers** in the Troop section. Create a fundraiser, then add each product with its selling unit, unit cost, and default price. For candy bought in cases of 60 for $30, use **bar**, cost **$0.50**, and the selling price per bar. Receive **120 units** for two cases. Use separate products/lots for different costs or pack sizes so historical margins remain unchanged.

Use **Record Activity** to receive stock, issue units to any scout or leader in People, record paid sales, accept unsold returns, record lost/damaged seller stock, and record money turned in. For transfers between sellers, return the units from the first seller and issue them to the second. A payment spanning several products is entered as one share per product. Actual sale totals can override the default price. These records cover paid product sales; customer credit orders and refunds are not modeled.

The summary shows revenue, gross profit on sold units, total inventory purchase cost, sales less all stock purchases, and remittances outstanding. Each seller shows quantities by product and their revenue, gross profit, money turned in, and outstanding amount. Gross profit excludes losses and overhead. Purchases and remittances do **not** automatically post to Transactions or scout accounts; enter the corresponding expense/receipt in the register using the activity notes to retain the reference.

Corrections use **Void** with a reason and retain the original history. Dependent activity must be voided first. Dated inventory validation prevents overselling, overissuing, excess returns, and over-remittance. Archive finished fundraisers to hide them from the active list; reopen to resume. People with fundraiser history cannot be deleted, but may be marked inactive.

Fundraiser, product, and activity tables are included in plaintext backup format 12, database reset, and integrity checks. The additive SwiftData models use CloudKit-compatible defaults and UUID references. Before a production release, initialize and deploy the new CloudKit record types using the established schema deployment workflow, then verify synchronization on a second device. Local tests do not verify production CloudKit sync. Concurrent offline edits on different devices can conflict; integrity checks report invalid merged inventory or payment balances.

## macOS software updates

TroopLedger includes Sparkle 2.10.0 for **Check for Updates…** in the app menu and **Preferences → Updates** for automatic checking. Updates require a configured HTTPS appcast and signed release archives. The macOS update-signing key stays in Keychain; iOS distribution is unchanged. See [the release guide](Scripts/SPARKLE-RELEASES.md) and `Scripts/prepare-sparkle-update.sh` for preparing updates from an exported, notarized app. The public repository’s permanent `updates` release hosts the appcast. The initial feed is empty until a signed, notarized application release is published.
